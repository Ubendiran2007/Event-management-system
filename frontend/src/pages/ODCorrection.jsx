...
const ODCorrection = () => {
    ...
    return (
    <Layout>
      <div className="flex-1 flex flex-col min-h-0 relative px-4 md:px-6 py-6 md:py-10">
                    ...
                    <div className="flex items-center gap-3 shrink-0 flex-wrap">
                        {!showForm && (
                            <div className="relative">
                                <button
                                    ...
                                    className="flex items-center gap-2 px-3 py-2 bg-white hover:bg-slate-50 border border-slate-200 rounded-xl text-[13px] font-bold text-slate-700 transition-colors shadow-sm"
                                >
                                    <SlidersHorizontal size={15} className="text-slate-500" />
                                    Filter: {activeTab === 'pending' ? 'Pending' : activeTab === 'approved' ? 'Approved' : 'Rejected'}
                        ...
                        {isStudent && !showForm && (
                            <button 
                                ...
                                className="bg-cse-accent text-white px-4 py-2 rounded-xl font-bold shadow-lg shadow-cse-accent/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center gap-2 text-sm"
                            >
                                <Send size={16} /> New Request
                            </button>
                        )}
                        <button onClick={() => navigate('/dashboard')} className="text-slate-500 hover:text-slate-800 font-bold flex items-center gap-1.5 transition-colors text-sm border border-slate-200 px-3 py-2 rounded-xl bg-white shadow-sm">
                              ...
                              <table className="w-full text-left border-collapse table-fixed">
                                <thead>
                                  <tr className="bg-slate-50/50 text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">
                                    <th className="py-3 sm:py-4 px-3 sm:px-6 w-[55%] sm:w-[35%]">STUDENT DETAILS</th>
                                    <th className="py-3 sm:py-4 px-3 sm:px-6 hidden sm:table-cell sm:w-[20%]">REQUESTED DATE</th>
                                    <th className="py-3 sm:py-4 px-3 sm:px-6 w-[25%] sm:w-[25%]">STATUS</th>
                                    <th className="py-3 sm:py-4 px-3 sm:px-6 w-[20%] sm:w-[20%] text-right">ACTIONS</th>
                                  </tr>
                                </thead>
                                </table>
                              </div>
                                      ...
                                      <td className="py-3 sm:py-4 px-3 sm:px-6 w-[55%] sm:w-[35%]">
                                         <div className="flex items-center gap-2 sm:gap-4">
                                            ...
                                            <div className="min-w-0">
                                               ...
                                               <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider mt-0.5 sm:hidden">
                                                  REQ: {new Date(req.createdAt).toLocaleDateString()}
                                               </p>
                                            </div>
                                         </div>
                                      </td>
                                      <td className="py-3 sm:py-4 px-3 sm:px-6 hidden sm:table-cell sm:w-[20%]">
                                         <p className="text-sm font-medium text-slate-700">
                                            {new Date(req.createdAt).toLocaleDateString()}
                                         </p>
                                      </td>
                                      <td className="py-3 sm:py-4 px-2 sm:px-6 w-[25%] sm:w-[25%]">
                                         <div className="scale-75 sm:scale-100 origin-left">
                                           {getStatusBadge(req.status)}
                                         </div>
                                      </td>
                                      <td className="py-3 sm:py-4 px-3 sm:px-6 w-[20%] sm:w-[20%] text-right">
                                         <div className="flex items-center justify-end gap-1 sm:gap-2">
                                            {isStaff && req.status.includes(currentUser.role.replace('_TEAM', '')) && activeTab === 'pending' && (
                                               <>
                                                  <button onClick={(e) => { e.stopPropagation(); openActionModal(req.id, 'APPROVE'); }} className="p-1.5 sm:p-2 bg-emerald-50 text-emerald-600 hover:bg-emerald-500 hover:text-white rounded-lg transition-colors shadow-sm" title="Approve">
                                                     <ShieldCheck size={16} className="sm:w-[18px] sm:h-[18px]" />
                                                  </button>
                                                  <button onClick={(e) => { e.stopPropagation(); openActionModal(req.id, 'REJECT'); }} className="p-1.5 sm:p-2 bg-red-50 text-red-600 hover:bg-red-500 hover:text-white rounded-lg transition-colors shadow-sm" title="Reject">
                                                     <XCircle size={16} className="sm:w-[18px] sm:h-[18px]" />
                                                  </button>
                                               </>
                                            )}
                                            <button className="p-1 sm:p-2 text-slate-400 hover:text-slate-600 transition-colors sm:ml-2 hidden sm:block">
