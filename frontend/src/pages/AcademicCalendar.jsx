...
const CalendarInput = ({ label, type, field, form, setForm, required = true }) => (
  <div className="space-y-1">
    ...
    {type === 'date' ? (
       <PremiumDatePicker value={form[field]} onChange={e => setForm({...form, [field]: e.target.value})} required={required} />
    ) : (
...
const AcademicCalendar = () => {
  ...
  // Removed blocking loading screen to allow optimistic rendering
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 sm:mb-8 gap-4 shrink-0">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 flex items-center gap-3">
            ...
            Academic Calendar
            {loading && <div className="w-5 h-5 border-2 border-indigo-200 border-t-indigo-600 rounded-full animate-spin ml-2"></div>}
          </h1>
            ...
            <form onSubmit={handleAddEvent} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <PremiumDatePicker required value={newEvent.date} onChange={e => setNewEvent({...newEvent, date: e.target.value})} className="w-full border-slate-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm border" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">End Date (Optional)</label>
                  <PremiumDatePicker value={newEvent.endDate} min={newEvent.date} onChange={e => setNewEvent({...newEvent, endDate: e.target.value})} className="w-full border-slate-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm border" />
                </div>
              </div>
              <div>
