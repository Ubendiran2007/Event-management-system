...
const NotificationCard = ({ notification }) => {
  ...
  return (
    <div className={`flex flex-col sm:flex-row gap-3 p-3 rounded-xl border transition-all ${isUnread ? 'bg-blue-50/30 border-blue-200 shadow-sm' : 'bg-white border-slate-200 hover:shadow-sm'}`}>
      <div 
        className="flex-1 cursor-pointer flex gap-3 items-start"
        onClick={handleClick}
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between mb-1">
            <h4 className={`text-sm leading-tight ${isUnread ? 'font-bold text-slate-900' : 'font-semibold text-slate-700'}`}>
              {notification.title}
            </h4>
            <div className="flex gap-2 items-center ml-2 flex-shrink-0">
              {notification.priority === 'CRITICAL' && (
                <span className="px-1.5 py-0.5 rounded-full bg-red-100 text-red-700 text-[10px] font-bold uppercase">Critical</span>
              )}
              {notification.priority === 'HIGH' && (
                <span className="px-1.5 py-0.5 rounded-full bg-orange-100 text-orange-700 text-[10px] font-bold uppercase">High</span>
              )}
      ...
      {/* Actions */}
      <div className="flex sm:flex-col justify-end sm:justify-start gap-1 border-t sm:border-t-0 sm:border-l border-slate-100 pt-2 sm:pt-0 sm:pl-2">
        {isUnread && (
          <button 
        ...
        {notification.status !== 'ARCHIVED' && (
          <button 
            ...
            className="p-1.5 text-slate-500 hover:bg-slate-100 rounded-lg transition-colors flex items-center justify-center"
            title="Archive"
          >
        ...
        <button 
          ...
          className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg transition-colors flex items-center justify-center"
          title="Delete"
        >
