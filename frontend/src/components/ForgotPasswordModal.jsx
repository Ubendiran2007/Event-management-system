...
const ForgotPasswordModal = ({ onClose }) => {
  ...
  const handleRequestOtp = async (e) => {
    ...
    setAlert(null);
    try {
      const res = await fetch((import.meta.env.VITE_BACKEND_URL || 'http://localhost:5001') + '/api/security/forgot-password', {
        method: 'POST',
  ...
  const handleResetPassword = async (e) => {
    ...
    setAlert(null);
    try {
      const res = await fetch((import.meta.env.VITE_BACKEND_URL || 'http://localhost:5001') + '/api/security/reset-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
